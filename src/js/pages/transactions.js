...
function render() {
  const container = el('notifications-dynamic-content');
  if (!container) return;

  if (loading) {
    container.innerHTML = '<div class="page-skeleton" style="margin: 2rem 0;"></div>';
    return;
  }
  ...
  const btnContainer = el('mark-all-btn-container');
  if (btnContainer) {
...
function init() {
  ...
  if (!pageShell) {
    pageShell = createPageShell('notifications-root', `
      ...
      <div id="notifications-dynamic-content"></div>
    `);
  }
...
onPageEnter('notifications', init);
